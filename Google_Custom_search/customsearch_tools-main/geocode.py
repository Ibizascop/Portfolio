...
import urllib.parse
from supply_updater import adresses as adrsx
...
class searcher:
    def __init__(self,item):
        ...
        qpage="https://maps.googleapis.com/maps/api/geocode/json?address="+search+"&language=fr&key=INSERT API KEY"
...
class searcher_detail:
    def __init__(self,item):
        self.item=item
        search=urllib.parse.quote_plus(self.item)
        qpage="https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input="+search+"&inputtype=textquery&fields=place_id,name,formatted_address&key=INSERT API KEY"
        query=urlreq.urlopen(qpage)
        self.data=query.read().decode('utf-8').replace('(','[').replace(')',']')
